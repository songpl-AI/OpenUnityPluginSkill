...
export function registerCompileTools(server, client, ws) {
    server.registerTool("unity_compile", {
        ...
        if (result?.errors?.length) {
            ...
            if (!errRes.ok)
                throw new Error(`Failed to get compile errors: ${errRes.error?.message}`);
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            return { content: [{ type: "text", text: `Compilation FAILED:\n${formatCompileErrors(errRes.data.errors)}` }] };
        }
    ...
    server.registerTool("unity_get_compile_errors", {
        ...
        await client.ensureConnected();
        const res = await client.get("/compile/errors", { type: type ?? "" });
        if (!res.ok)
            throw new Error(`Unity API Error [${res.error?.code}]: ${res.error?.message}`);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const formatted = formatCompileErrors(res.data.errors);
        return { content: [{ type: "text", text: `Compile status: ${res.data.status}\n${formatted}` }] };
    });
    server.registerTool("unity_get_console_logs", {
        ...
        await client.ensureConnected();
        const res = await client.get("/console/logs", { type: type ?? "", limit: limit ?? 50 });
        if (!res.ok)
            throw new Error(`Unity API Error [${res.error?.code}]: ${res.error?.message}`);
        if (!res.data.logs.length)
            return { content: [{ type: "text", text: "Console is empty." }] };
        return { content: [{ type: "text", text: res.data.logs.map(l => `[${l.type.toUpperCase()}] ${l.message}`).join("\n") }] };
    });
}
