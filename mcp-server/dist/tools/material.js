import { z } from "zod";
export function registerMaterialTools(server, client) {
    // ── Render Pipeline ─────────────────────────────────────────────────────────
    server.registerTool("unity_get_render_pipeline", {
        ...
        const res = await client.get("/material/render-pipeline");
        if (!res.ok)
    ...
    server.registerTool("unity_set_material_properties", {
        ...
        inputSchema: {
            ...
            properties: z.record(z.unknown()).describe("Map of shader property name → value. " +
                "Examples: { \"_BaseColor\": [1, 0.2, 0, 1], \"_Metallic\": 0.8, \"_BaseMap\": \"Assets/Textures/Rock.png\" }"),
        },
    }, async ({ path, properties }) => {
        await client.ensureConnected();
        const res = await client.post("/material/properties", { path, properties });
        if (!res.ok)
            throw new Error(`Unity API Error [${res.error?.code}]: ${res.error?.message}`);
        const lines = [
    ...
    server.registerTool("unity_assign_material", {
        ...
        await client.ensureConnected();
        const res = await client.post("/material/assign", { goPath, materialPath, slot: slot ?? 0 });
        if (!res.ok)
            throw new Error(`Unity API Error [${res.error?.code}]: ${res.error?.message}`);
        return {
            content: [{
