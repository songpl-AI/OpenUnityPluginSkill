...
export function registerGameObjectTools(server, client) {
    ...
    server.registerTool("unity_delete_gameobject", {
        ...
        if (!res.ok)
            throw new Error(`Unity API Error [${res.error?.code}]: ${res.error?.message}`);
        return { content: [{ type: "text", text: `Deleted GameObject: ${path}` }] };
    });
    server.registerTool("unity_set_transform", {
        ...
        if (!res.ok)
            throw new Error(`Unity API Error [${res.error?.code}]: ${res.error?.message}`);
        return { content: [{ type: "text", text: `Transform updated for: ${params.path}` }] };
    });
    server.registerTool("unity_find_gameobjects", {
        ...
        await client.ensureConnected();
        const res = await client.get("/gameobject", { name: name ?? "", tag: tag ?? "" });
        if (!res.ok)
            throw new Error(`Unity API Error [${res.error?.code}]: ${res.error?.message}`);
        if (!res.data.count)
            return { content: [{ type: "text", text: "No GameObjects found matching criteria." }] };
        return { content: [{ type: "text", text: res.data.objects.map(o => `• ${o.path} (tag: ${o.tag}, active: ${o.active})`).join("\n") }] };
    });
    server.registerTool("unity_get_components", {
        ...
        await client.ensureConnected();
        const res = await client.get(`/gameobject/${encodeGoPath(path)}/components`);
        if (!res.ok)
            throw new Error(`Unity API Error [${res.error?.code}]: ${res.error?.message}`);
        return { content: [{ type: "text", text: `Components on '${path}':\n${res.data.components.map(c => `• ${c}`).join("\n")}` }] };
    });
    ...
    server.registerTool("unity_set_component_property", {
        ...
        await client.ensureConnected();
        const res = await client.post(`/gameobject/${encodeGoPath(path)}/component/${encodeURIComponent(componentType)}/values`, { values: properties });
        if (!res.ok)
            throw new Error(`Unity API Error [${res.error?.code}]: ${res.error?.message}`);
        return { content: [{ type: "text", text: `Updated properties on '${componentType}' of '${path}': ${Object.keys(properties).join(", ")}` }] };
    });
}
