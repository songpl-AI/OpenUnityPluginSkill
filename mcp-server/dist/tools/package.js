import { z } from "zod";
export function registerPackageTools(server, client) {
    server.registerTool("unity_list_packages", {
        ...
        const res = await client.get("/package/list", { includeBuiltIn: includeBuiltIn ? "true" : "false" });
        if (!res.ok)
        ...
        return {
            content: [{ type: "text", text: `Installed packages (${res.data.count}):\n${lines.join("\n")}` }]
        };
    });
    server.registerTool("unity_install_package", {
        ...
        inputSchema: {
            packageId: z.string().describe("Package identifier, optionally with version (e.g. 'com.unity.textmeshpro' or 'com.unity.textmeshpro@3.0.6')"),
        },
    }, async ({ packageId }) => {
        await client.ensureConnected();
        // Package install blocks C# side for up to 120s — use extended timeout
        const res = await client.post("/package/add", { packageId }, { timeoutMs: 130_000 });
        if (!res.ok)
            throw new Error(`Unity API Error [${res.error?.code}]: ${res.error?.message}`);
        return {
            content: [{
    ...
    server.registerTool("unity_remove_package", {
        ...
        await client.ensureConnected();
        const res = await client.post("/package/remove", { packageId }, { timeoutMs: 70_000 });
        if (!res.ok)
            throw new Error(`Unity API Error [${res.error?.code}]: ${res.error?.message}`);
        return {
            content: [{
