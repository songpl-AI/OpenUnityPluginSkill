...
export function registerProjectTools(server, client) {
    ...
    server.registerTool("unity_get_scripts", {
        ...
        if (res.data.degraded) {
            return { content: [{ type: "text", text: `⚠ ${res.data.reason}\nScript files:\n${(res.data.files ?? []).map(f => `• ${f}`).join("\n")}` }] };
        }
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return { content: [{ type: "text", text: `Found ${res.data.types?.length ?? 0} types:\n${formatScriptTypes(res.data.types)}` }] };
    });
    server.registerTool("unity_find_assets", {
        ...
        if (!res.ok)
            throw new Error(`Unity API Error [${res.error?.code}]: ${res.error?.message}`);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return { content: [{ type: "text", text: `Found ${res.data.count} assets:\n${formatAssetList(res.data.assets)}` }] };
    });
}
