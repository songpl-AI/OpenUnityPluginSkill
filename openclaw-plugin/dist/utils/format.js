...
function formatCompileErrors(errors) {
    if (errors.length === 0)
        return "No compile errors.";
    return errors.map(e => `[${e.type.toUpperCase()}] ${e.file}:${e.line}:${e.column}\n  ${e.message}`).join("\n");
}
function formatScriptTypes(types) {
    if (types.length === 0)
        return "No user scripts found.";
    return types.map(t => {
