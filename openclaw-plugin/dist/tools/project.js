        ...
        execute: async (_toolCallId) => {
            try {
                ...
                if (res.data.degraded) {
                    return (0, format_1.textResult)(`⚠ ${res.data.reason}\nScript files:\n${(res.data.files ?? []).map(f => `• ${f}`).join("\n")}`);
                }
                // TODO: replace with SDK types
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                return (0, format_1.textResult)(`Found ${res.data.types?.length ?? 0} types:\n${(0, format_1.formatScriptTypes)(res.data.types)}`);
            }
            catch (err) {
                return (0, error_1.handleError)(err);
            }
    ...
    api.registerTool({
        ...
        execute: async (_toolCallId, params) => {
            try {
                ...
                // TODO: replace with SDK types
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                return (0, format_1.textResult)(`Found ${res.data.count} assets:\n${(0, format_1.formatAssetList)(res.data.assets)}`);
            }
            catch (err) {
                return (0, error_1.handleError)(err);
            }
